...
public record IncidentTopicResponse(
        ...
        @Schema(description = "Responsible agent (id + name)", nullable = true) LookupResponse assignedAgent
) {
    public static IncidentTopicResponse from(IncidentType type) {
        LookupResponse category = type.getCategory() != null
                ...
                : null;
        LookupResponse assignedAgent = type.getAgent() != null
                ? LookupResponse.from(
                        type.getAgent().getId(),
                        type.getAgent().getUser() != null ? type.getAgent().getUser().getFullName() : type.getAgent().getUserId())
                : null;
        return new IncidentTopicResponse(type.getId(), type.getName(), type.getDescription(), category, assignedAgent);
    }
}
