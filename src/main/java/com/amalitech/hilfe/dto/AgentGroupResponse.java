...
public record AgentGroupResponse(
    ...
    public static AgentGroupResponse from(AgentGroup group, LookupResponse primaryAgent, long memberCount) {
        LookupResponse department = group.getDepartment() != null
                ? LookupResponse.from(group.getDepartment().getId(), group.getDepartment().getName())
                : null;
        return from(group, department, primaryAgent, memberCount);
    }

    public static AgentGroupResponse from(
            AgentGroup group,
            LookupResponse department,
            LookupResponse primaryAgent,
            long memberCount
    ) {
